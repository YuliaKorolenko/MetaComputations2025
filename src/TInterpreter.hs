...
turingInterpreter = program ["Q", "Right"]
                    [
                        bl  "init"  ["QTail" #= "Q", "Left" #= Constant (List [])],   -- init: QTail := Q; Left := '();
                        blj "loop"  (if' (v "QTail" ?= Constant (List [])) "stop" "cont"),                   -- "loop: if QTail = '() goto stop else cont;"
                        ...
                        bj          (if' (v "Operator" ?= "right") "do-right" "cont1"),  -- if Operator = 'right goto do-right else cont1;
                        blj "cont1" (if' (v "Operator" ?= "left") "do-left" "cont2"),    -- cont1: if Operator = 'left goto do-left else cont2;     
                        blj "cont2" (if' (v "Operator" ?= "write") "do-write" "cont3"),  -- cont2: if Operator = 'write goto do-write else cont3;   
                        blj "cont3" (if' (v "Operator" ?= "goto") "do-goto" "cont4"),    -- cont3: if Operator = 'goto goto do-goto else cont4;   
                        blj "cont4" (if' (v "Operator" ?= "if") "do-if" "error"),         -- cont4: if Operator = 'if goto do-if else error;  
